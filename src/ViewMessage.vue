<template>
  <div class="inbox-body">
    <div class="mail-option">
      <button class="btn btn-secondary mb-1" @click="navigateBack">
      ...
      <template v-if="typeof data.message.isRead !== 'undefined'">
        <button class="btn btn-secondary" @click="data.message.isRead = false" :disabled="!data.message.isRead">
          <i class="fa fa-envelope-open" aria-hidden="true"></i>&nbsp; Mark as unread
        </button>

        <button class="btn btn-secondary" @click="data.message.isRead = true" :disabled="data.message.isRead">
    ...
    <div v-if="data.message.attachments.length > 0" class="attachments">
      <h4>Attachments</h4>
      <ul v-for="attachment in data.message.attachments">
        <i class="fa-fa-paperclip">{{ attachment.fileName }}</i> ({{ attachment.size | formatBytes }})
      </ul>
    </div>
